...
namespace Microsoft.DotNet.Cli.Build
{
    public class PrepareTargets
    {
        ...
        public static BuildTargetResult Init(BuildTargetContext c)
        {
            ...
            // On non-Windows we need to figure out where Mono keeps the reference assemblies
            if (!RuntimeInformation.IsOSPlatform(OSPlatform.Windows) && string.IsNullOrEmpty(Environment.GetEnvironmentVariable("DOTNET_REFERENCE_ASSEMBLIES_PATH")))
            {
                if (RuntimeInformation.IsOSPlatform(OSPlatform.OSX) && Directory.Exists(Dirs.OSXReferenceAssembliesPath))
                {
                    Environment.SetEnvironmentVariable("DOTNET_REFERENCE_ASSEMBLIES_PATH", Dirs.OSXReferenceAssembliesPath);
                }
                else if (Directory.Exists(Dirs.UsrLocalReferenceAssembliesPath))
                {
                    Environment.SetEnvironmentVariable("DOTNET_REFERENCE_ASSEMBLIES_PATH", Dirs.UsrLocalReferenceAssembliesPath);
                }
                else if (Directory.Exists(Dirs.UsrReferenceAssembliesPath))
                {
                    Environment.SetEnvironmentVariable("DOTNET_REFERENCE_ASSEMBLIES_PATH", Dirs.UsrReferenceAssembliesPath);
                }
            }
